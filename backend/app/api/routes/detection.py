...
async def analyze_flow(payload: AnomalyCreate, service: DetectionService = Depends(get_detection_service)):
...
async def list_anomalies(page: int = Query(1, ge=1), page_size: int = Query(DEFAULT_PAGE_SIZE, ge=1, le=MAX_PAGE_SIZE), is_anomalous: Optional[bool] = None, status: Optional[str] = None, src_ip: Optional[str] = None, service: DetectionService = Depends(get_detection_service)):
...
async def get_anomaly(anomaly_id: str, service: DetectionService = Depends(get_detection_service)):
...
async def update_anomaly(anomaly_id: str, payload: AnomalyUpdate, service: DetectionService = Depends(get_detection_service)):
