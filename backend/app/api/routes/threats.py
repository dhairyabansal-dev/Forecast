...
from app.api.dependencies import get_current_user, get_threat_service, require_roles
...
async def create_threat(payload: ThreatCreate, user: User = Depends(require_roles("ADMIN", "DATA_SCIENTIST")), service: ThreatService = Depends(get_threat_service)):
...
async def update_threat(threat_id: str, payload: ThreatUpdate, user: User = Depends(require_roles("ADMIN", "DATA_SCIENTIST")), service: ThreatService = Depends(get_threat_service)):
...
async def resolve_threat(threat_id: str, user: User = Depends(require_roles("ADMIN", "DATA_SCIENTIST", "ANALYST")), service: ThreatService = Depends(get_threat_service)):
...
async def delete_threat(threat_id: str, user: User = Depends(require_roles("ADMIN")), service: ThreatService = Depends(get_threat_service)):
