...
async def create_evidence(payload: EvidenceCreate, user: User = Depends(require_roles("ADMIN", "DATA_SCIENTIST")), session: AsyncSession = Depends(get_db_session), blockchain: BlockchainService = Depends(get_blockchain_service)):
    repo = EvidenceRepository(session); content_hash = blockchain.compute_evidence_hash(payload.payload)
    ...
    await record_audit(session, action="EVIDENCE_CREATION", status="SUCCESS", user_id=user.id, resource=getattr(created, "id", None))
    return created
...
async def anchor_evidence(evidence_id: str, user: User = Depends(require_roles("ADMIN", "DATA_SCIENTIST")), session: AsyncSession = Depends(get_db_session), blockchain: BlockchainService = Depends(get_blockchain_service)):
    repo = EvidenceRepository(session); evidence = await repo.get_by_id(evidence_id)
    if evidence is None: raise HTTPException(status_code=404, detail="Evidence not found")
    try: result = blockchain.anchor_evidence(evidence.content_hash)
    except RuntimeError as e: raise HTTPException(status_code=503, detail=str(e))
    updated = await repo.update(evidence, status=EvidenceStatus.ANCHORED.value, blockchain_tx_hash=result["tx_hash"], blockchain_block_number=result["block_number"], anchored_at=result["anchored_at"])
    await record_audit(session, action="EVIDENCE_ANCHOR", status="SUCCESS", user_id=user.id, resource=evidence_id)
    return updated
...
async def verify_evidence(evidence_id: str, user: User = Depends(get_current_user), session: AsyncSession = Depends(get_db_session), blockchain: BlockchainService = Depends(get_blockchain_service)):
    ...
    try: is_valid = blockchain.verify_evidence(evidence.content_hash)
    except RuntimeError as e: raise HTTPException(status_code=503, detail=str(e))
...
async def list_evidence(page: int = Query(1, ge=1), page_size: int = Query(DEFAULT_PAGE_SIZE, ge=1, le=MAX_PAGE_SIZE), threat_id: str | None = None, status: str | None = None, user: User = Depends(get_current_user), session: AsyncSession = Depends(get_db_session)):
    repo = EvidenceRepository(session); items, total = await repo.list(page=page, page_size=page_size, threat_id=threat_id, status=status)
