...
async def create_evidence(payload: EvidenceCreate, session: AsyncSession = Depends(get_db_session), blockchain: BlockchainService = Depends(get_blockchain_service)):
...
async def anchor_evidence(evidence_id: str, session: AsyncSession = Depends(get_db_session), blockchain: BlockchainService = Depends(get_blockchain_service)):
    repo = EvidenceRepository(session)
...
async def verify_evidence(evidence_id: str, session: AsyncSession = Depends(get_db_session), blockchain: BlockchainService = Depends(get_blockchain_service)):
    repo = EvidenceRepository(session)
    evidence = await repo.get_by_id(evidence_id)
    if evidence is None:
        raise HTTPException(status_code=404, detail="Evidence not found")
...
async def list_evidence(page: int = Query(1, ge=1), page_size: int = Query(DEFAULT_PAGE_SIZE, ge=1, le=MAX_PAGE_SIZE), threat_id: str | None = None, status: str | None = None, session: AsyncSession = Depends(get_db_session)):
    repo = EvidenceRepository(session)
...
async def get_evidence(evidence_id: str, session: AsyncSession = Depends(get_db_session)):
    repo = EvidenceRepository(session)
    evidence = await repo.get_by_id(evidence_id)
    if evidence is None:
        raise HTTPException(status_code=404, detail="Evidence not found")
