...
async def verify_evidence(
    ...
    if evidence.status != EvidenceStatus.ANCHORED.value:
        return EvidenceVerifyResponse(
            ...
            is_valid=False,
            on_chain_hash=None,
            local_hash=evidence.content_hash,
            checked_at=utc_now(),
            message="Evidence has not been anchored on-chain yet.",
        )
    ...
    return EvidenceVerifyResponse(
        evidence_id=evidence_id,
        is_valid=is_valid,
        on_chain_hash=evidence.content_hash if is_valid else None,
        local_hash=evidence.content_hash,
        checked_at=utc_now(),
        message="Verified on-chain." if is_valid else "Hash mismatch or not found on-chain.",
    )
